...
import {
  getAllOrdersByEnterpriseFirebaseUid,
  updateOrderUrl,
} from "../../routes/routes";
...
function productRatingComparator(a, b) {
  if (
    Math.floor(a["average rating score"]) <
    Math.floor(b["average rating score"])
  ) {
    return 1;
  }
  if (
...
const useDashboardHooks = (user) => {
  ...
  useEffect(() => {
    axios
      .get(getAllOrdersByEnterpriseFirebaseUid + user.uid)
      .then((response) => {
        ...
        records.forEach((r) => {
          ...
          if (orderYear == currentYear && orderMonth == selectedMonth) {
            let currentDate = Math.floor(
              r.dateOfOrder.split("T")[0].split("-")[2]
            );
